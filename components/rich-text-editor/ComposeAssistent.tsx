...
export const ComposeAssistent = ({ content, onAccept }: ComposeAssistentProps) => {
    ...
    function handleOpenChange(nextOpen: boolean) {
        ...
        if (nextOpen) {
            const hasAssistanrMessage = messages.some((m) => m.role === 'assistant')

            if (status !== 'ready' || hasAssistanrMessage) {
                return
            }
    ...
    return (
        <Popover open={open} onOpenChange={handleOpenChange}>
            <PopoverTrigger asChild>
                <Button type="button" size='sm' className="relative overflow-hidden rounded-full bg-gradient-to-t  from-violet-600 to-fuchsia-600 text-white shadow-md hover:shadow-lg focus-visible:ring-2 focus-visible:ring-ring">
            ...
            <PopoverContent className="w-[25rem] p-0">
                <div className="flex items-center justify-between px-4 py-3 border-b">
                    <div className="flex items-center gap-2">
                        <span className="relative inline-flex items-center justify-center rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-600 py-1.5 px-4 gap-1.5">
                            ...
                            <span className="text-sm  font-medium">Compose Assistent (Preview)</span>
                        </span>
                    </div>
                ...
                <div className="flex items-center justify-end gap-3 border-t px-3 py-2 bg-muted/30">
                    ...
                    <Button disabled={!composedText} type="submit" size='sm' onClick={() => {
