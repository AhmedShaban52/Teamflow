...
import { tibTapJsonToMarkdown } from "@/lib/json-to-markdown";
...
export const generateThreadSummary = base
  ...
  .handler(async ({ input, context, errors }) => {
    const basedMessage = await prisma.message.findFirst({
      where: {
        id: input.MeaasgeId,
        Channel: {
    ...
    const parentId = basedMessage.threadId ?? basedMessage.id;
    ...
    const parentText = await tibTapJsonToMarkdown(parent.content);

    const lines = [];

    lines.push(
      `Thead Root - ${parent.authorName} - ${parent.createdAt.toISOString()}`
    );
    ...
    if (replies.length > 0) {
      ...
      for (const r of replies) {
        const t = await tibTapJsonToMarkdown(r.content);
...
export const generateCompose = base
  ...
  .handler(async ({ input }) => {
    const markdown = await tibTapJsonToMarkdown(input.content);
    ...
    const result = streamText({
      ...
      messages: [
        {
          role: "user",
          content: "Please rewrite and improve the follwing content:",
        },
        {
          role: "user",
          content: markdown,
