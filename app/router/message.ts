...
export const createMessage = base
  ...
  .handler(async ({ input, context, errors }) => {
    // verify the channel belnotes the users organization
    ...
    if (input.threadId) {
      const perntMessage = await prisma.message.findFirst({
        where: {
          id: input.threadId,
          Channel: {
            workspaceId: context.workspace.orgCode,
      ...
      if (
        !perntMessage ||
        perntMessage.channelId !== input.channelId ||
        perntMessage.threadId !== null
      ) {
