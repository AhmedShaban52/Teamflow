...
export function MessageList() {
  ...
  const [hasInitialScolled, setHasInitialScolled] = useState(false)
  ...
  const infiniteOptions = orpc.message.list.infiniteOptions({
    input: (pageParam: string | undefined) => ({
      channelId: channelId,
      cursor: pageParam,
      limit: 10  // controll the number of messages to be loaded
    }),
  ...
  useEffect(() => {
    if (!hasInitialScolled && data?.pages.length) {
      const el = scrollRef.current

      if (el) {
        bottomRef.current?.scrollIntoView({ block: 'end' })
        setHasInitialScolled(true)
        setIsAtBottom(true)
      }
    }
  }, [hasInitialScolled, data?.pages.length])
  ...
  useEffect(() => {
    ...
    const scrollToBottomIfNedded = () => {
      if (isAtBottom || !hasInitialScolled) {
  ...
  useEffect(() => {
    ...
    const prvLastId = lastItemIdRef.current

    const el = scrollRef.current

    if (prvLastId && lastId !== prvLastId) {
