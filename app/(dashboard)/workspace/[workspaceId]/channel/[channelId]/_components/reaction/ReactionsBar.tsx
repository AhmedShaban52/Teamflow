...
type InfinityReplies = InfiniteData<MessagePage>
    ...
    const toggleMutation = useMutation(
        orpc.message.reaction.toggle.mutationOptions({
            onMutate: async (vars: { messageId: string; emoji: string }) => {
                const bumb = (rxns: GroupedReactionSchemaType[]) => {
                    ...
                    queryClient.setQueryData(listOptions.queryKey, (old) => {
                        ...
                        if (vars.messageId === context.threadId){
                            return {
                                ...
                                parent:{
                                    ...old.parent,
                                    reactions: bumb(old.parent.reactions)
                                }
                            }
                        }
                        return {
                            ...old,
                            messages: old.messages.map((m) => m.id === vars.messageId ? {...m, reactions: bumb(m.reactions)} : m)
                        }
                    })
                ...
                queryClient.setQueryData<InfinityReplies>(
                    listKey,
                    (old) => {
                        if (!old) return old
