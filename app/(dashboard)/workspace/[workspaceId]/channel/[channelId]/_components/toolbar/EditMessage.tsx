    ...
    const updateMutation = useMutation(
        orpc.message.update.mutationOptions({
            onSuccess: (updated) => {
                ...
                type InfinityMessages = InfiniteData<MessagePage>

                queryClient.setQueryData<InfinityMessages>(["message.list", message.channelId], (old) => {
                    if (!old) return old
            ...
            <form onSubmit={form.handleSubmit(onSubmit)}>
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormControl>
                                <RichTextEditor field={field} sendButton={
                                    <div className="flex items-center gap-4">
                                        <Button disabled={updateMutation.isPending} onClick={onCancel} type="button" size='sm' variant='outline'>Cancel</Button>
