...
import { useThread } from "@/providers/ThredProvider";
...
export function ThreadSidebar({ user }: ThreadSidebarProps) {
    const { selectedThredId, closeThred } = useThread()
    ...
    const { data, isLoading } = useQuery(
        orpc.message.thread.list.queryOptions({
            input: {
                messageId: selectedThredId!
            },
            enabled: Boolean(selectedThredId)
        })
    )
    ...
    useEffect(() => {
        ...
        // MutationObserver watches for DOM changes (eg. images, loading, contentuUdate...etc)
    ...
    return (
        <div className="w-[30rem] border-l flex flex-col h-full">
            ...
            <div className="border-b h-14 px-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <SummarizeThread messageId={selectedThredId!}/>
                    <Button onClick={closeThred} variant='outline' size='icon'>
                        <X className="size-4" />
                    </Button>
                </div>
            </div>
                ...
                <div ref={scrollRef} onScroll={handleScroll} className="h-full overflow-y-auto">
                    {data && (
                        <>
                                ...
                                <div className="flex space-x-3">
                                    ...
                                    <div className="flex-1 space-y-1 min-w-0">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-xs text-muted-foreground">{new Intl.DateTimeFormat('en-US', {
                                                hour: 'numeric',
                                                minute: 'numeric',
                                                hour12: true,
                                                month: 'short',
                            ...
                            {/* Thread Relpies */}
                            <div className="p-2">
                                <p className="text-xs text-muted-foreground mb-3 px-2 ">{data.messages.length} replies</p>
                                <div className="space-y-1">
                                    {data.messages.map((reply) => (
                                        <ThreadReply selectedThredId={selectedThredId!} key={reply.id} message={reply} />
                                    ))}
                                </div>
                            </div>
            ...
            <div className="border-t p-4">
                <ThreadReplyForm user={user} threadId={selectedThredId!} />
            </div>
        </div>
    )
}